...
float draw( in vec2 st , in float scale, in float move, in float theshold){
    ...
    for(int i = 0; i < 5; i++){
        vec3 color_t = vec3(0.);
        vec2 noisest = st * pow(scale,float(i)) +move*float(i) + cos(u_time/10.*float(i) + move);
        color_t =vec3(smoothstep(0.120,.9,noise(noisest))) - vec3(smoothstep(0.464,.8,noise(noisest)));	
        color_all+= color_t*.7;
...
void main() {
    ...
    mouseMove.x *= u_resolution.x/u_resolution.y; 
    st.x -=0.;
    st.x *= u_resolution.x/u_resolution.y;  
    uv.x *= u_resolution.x/u_resolution.y;
    float interact = mouseEffect(st, mouseMove, .02)*.25;  
    
    vec4 info = texture2D(u_tex0,st); // 讀取圖檔（u_tex0），以uv的方式讀
    ...
    vec3 color = vec3(0.);
    vec3 colorAll;
    colorAll.r = draw((st+interact)*10.,1.20,205.416,pic.r);
    colorAll.g = draw((st+interact*2.)*5.,.90,120.,pic.g);
    colorAll.b = draw((st+interact*.5)*2.,1.120,5000.,1.-pic.b);
